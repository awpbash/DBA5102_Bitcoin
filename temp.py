...
def load_all_models(models_config):
    ...
    for name, config in models_config.items():
        try:
            # --- UPDATED: Logic to load LSTM model and scalers ---
...
def fetch_and_prepare_data(num_lags=21):
    ...
    # --- NEW: Add the target variable used during training ---
            ...
            if model_config['type'] == 'linear':
                ...
                y_pred = pd.Series(y_pred, index=df.index)
                    ...
                    if required_data_start_loc < 0:
                        st.warning(f"Not enough historical data for LSTM (needs {n_steps} prior days). Skipping.")
                        continue

                    data_for_lstm = df_full.iloc[required_data_start_loc:].copy()
                    
                    x_scaler = model_config["x_scaler_obj"]
                    y_scaler = model_config["y_scaler_obj"]

                    # Scale the features (X)
                    scaled_features = x_scaler.transform(data_for_lstm[lstm_feature_cols])
                    
                    # --- FIX: Use 'Next_Return' to match the training script ---
                    scaled_target = y_scaler.transform(data_for_lstm[['Next_Return']])

                    # Combine them back into a single NumPy array
